    ...
    <div class="userManage">
        ...
        <el-form :inline="true" class="demo-form-inline search-style" size="small">
            ...
            <el-form-item>
                ...
                <el-button type="primary" @click.native="initRoleData()">维护角色</el-button>
            </el-form-item>
        </el-form>
                    ...
                    >
                        <template slot-scope="scope">
                            ...
                            <el-button type="danger" icon="el-icon-delete" size="mini"
                                       @click.native="sureView(delUser,tableData[scope.$index]['user_id'],tableData[scope.$index]['userName']+'用户')">
                                删除
                            </el-button>
                        </template>
                    </el-table-column>
        ...
        >
            <el-form>
                ...
                <el-form-item label="密码" :label-width="userData.formLabelWidth"
                              prop="desc">
                    <el-input v-model="userData.password" type="password" style="max-width: 450px"
                              :disabled="!userData.statusPassword">
                    </el-input>
        ...
        <el-dialog
                title="角色设置"
                :visible.sync="editRoleData.modelFormVisible"
                width="30%"
        >
            <el-form>

                <el-form-item label="角色名" :label-width="editRoleData.formLabelWidth">
                    <el-input v-model="editRoleData.role_name" placeholder="请填写角色名">
                    </el-input>
                    <el-button type="primary" @click.native="saveRoleData()" size="small">添 加</el-button>
                </el-form-item>
            </el-form>
            <el-table :data="roleData" stripe v-model="roleData">
                <el-table-column
                        prop="role_id"
                        label="id"
                >
                ...
                >
                    <template slot-scope="scope">
                        <el-button v-show="roleData[scope.$index]['role_id']>2"
                                   type="danger" icon="el-icon-delete" size="mini"
                                   @click.native="sureView(delRole,roleData[scope.$index]['role_id'],roleData[scope.$index]['role_name'])"
                        >
                            删除
                        </el-button>
                    </template>
            ...
            <span slot="footer" class="dialog-footer">
                    <el-button @click="editRoleData.modelFormVisible = false" size="small">取 消</el-button>
                </span>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initRoleData() {
                this.editRoleData.role_name = '';
                this.editRoleData.role_id = null;
                this.editRoleData.modelFormVisible = true;
            },
            editUser(id) {
            ...
            saveRoleData() {
                if (!this.editRoleData.role_name) {
                    this.$message({
                        showClose: true,
                        message: '请填写角色名',
                        type: 'warning',
                ...
                this.$axios.post(this.$api.addRoleApi, {
                    'role_name': this.editRoleData.role_name,
                    'role_id': this.editRoleData.role_id,
                }).then((response) => {
                        if (response.data['status'] === 0) {
                            this.$message({
                                showClose: true,
            ...
            delRole(id) {
                this.$axios.post(this.$api.delRoleApi, {'role_id': id}).then((response) => {
                        this.messageShow(this, response);
                        this.findUser();
                    }
                )
