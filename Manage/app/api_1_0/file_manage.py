...
def api_upload_pic():
    ...
    for file in files:
        ...
        name = str(int(time.time())) + str(index) + str(current_user.id) + '.' + suffix
        file.save(os.path.join(NOTES_ADDRESS, name))
        image_urls.append('/notes/{}'.format(name))
        index = index + 1
    return jsonify({'data': image_urls, "msg": "上传成功", "status": 1})
